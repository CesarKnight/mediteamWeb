...
<head>
    ...
    <style>
        ...
        .badge { display: inline-block; padding: 1px 6px; border-radius: 3px; font-size: 9px; color: #fff; background: #059669; }

        .sin-tratamientos { padding: 20px; text-align: center; color: #9ca3af; font-style: italic; }
...
<body>
    ...
    @if ($tratamientos->isEmpty())
        <div class="sin-tratamientos">
            No se encontraron tratamientos para el paciente en el periodo seleccionado.
        </div>
    @else
        <table class="eventos">
            <thead>
                <tr>
                    ...
                    <th style="width: 70px;">Historia</th>
                    <th>Médico</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($tratamientos as $tratamiento)
                    <tr>
                        <td>{{ $tratamiento['created_at']->format('d/m/Y H:i') }}</td>
                        <td><span class="badge">Tratamiento</span> {{ $tratamiento['medicamento'] }}</td>
                        <td>cada {{ $tratamiento['frecuencia_horas'] }} horas</td>
                        <td>#{{ $tratamiento['historia']['id'] }} ({{ $tratamiento['historia']['estado'] }})</td>
                        <td>
                            {{ $tratamiento['medico']['name'] }} {{ $tratamiento['medico']['lastName'] }}
                            @if ($tratamiento['medico']['especialidad'])
                                &mdash; {{ $tratamiento['medico']['especialidad'] }}
                            @endif
                        </td>
                    </tr>
                @endforeach
