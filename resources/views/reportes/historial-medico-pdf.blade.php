...
<head>
    <meta charset="utf-8">
    <title>Historial de médico</title>
    <style>
        ...
        .badge-anulado { background: #dc2626; }
        .badge-finalizado { background: #059669; }

        .counts span { display: inline-block; margin-right: 8px; color: #4b5563; }
...
<body>
    ...
    @if ($historias->isEmpty())
        <div class="sin-historias">
            No se encontraron historias clínicas en las que el médico haya estado involucrado en el periodo seleccionado.
        </div>
    @else
        <table class="eventos">
            <thead>
                <tr>
                    ...
                    <th style="width: 60px;">Historia</th>
                    <th style="width: 60px;">Estado</th>
                    <th>Paciente</th>
                    <th>Médico creador</th>
                    <th style="width: 130px;">Registros</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($historias as $historia)
                    <tr>
                        <td>{{ $historia['fechaInvolucramiento']->format('d/m/Y H:i') }}</td>
                        <td>#{{ $historia['id'] }}</td>
                        <td><span class="badge badge-{{ $historia['estado'] }}">{{ $historia['estado'] }}</span></td>
                        <td>{{ $historia['paciente']['name'] }} {{ $historia['paciente']['lastName'] }} (CI {{ $historia['paciente']['ci'] }})</td>
                        <td>{{ $historia['medicoCreador']['name'] }} {{ $historia['medicoCreador']['lastName'] }}</td>
                        <td class="counts">
                            <span>{{ $historia['totalConsultas'] }} consultas</span><br>
                            <span>{{ $historia['totalDiagnosticos'] }} diag.</span><br>
                            <span>{{ $historia['totalTratamientos'] }} trat.</span>
                        </td>
                    </tr>
                @endforeach
            </tbody>
